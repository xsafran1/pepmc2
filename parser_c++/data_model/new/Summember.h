...
const Summember<T> Summember<T>::operator*(/*const*/ Summember<T> & s2)
{
	...
	for (std::vector<ramp>::iterator it = ramps1.begin(); it != ramps1.end(); it++) {
		s_final.AddRamp(*it);
	}

	std::vector<ramp> ramps2 = s2.GetRamps();
	for (std::vector<ramp>::iterator it = ramps2.begin(); it != ramps2.end(); it++) {
		s_final.AddRamp(*it);
	}

	std::vector<sigmoid> sigmoids1 = s1.GetSigmoids();
	for (std::vector<sigmoid>::iterator it = sigmoids1.begin(); it != sigmoids1.end(); it++) {
		s_final.AddSigmoid(*it);
	}

	std::vector<sigmoid> sigmoids2 = s2.GetSigmoids();
	for (std::vector<sigmoid>::iterator it = sigmoids2.begin(); it != sigmoids2.end(); it++) {
		s_final.AddSigmoid(*it);
	}
