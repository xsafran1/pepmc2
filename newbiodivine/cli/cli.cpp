...
int main(int argc, char** argv) {
	...
	switch(pid1 = fork()) {
		case 0:
			...
			close(fd[1]);
			execl("../bin/divine.combine","divine.combine",modelFile.c_str(),propertyFile.c_str());
			_exit(EXEC_FAILED);
	...
	for(int a = 0; a < inputFiles.size(); a++) {
		...
		switch(pid = fork()) {
			...
			case 0:		// in the Children process
				...
				case OWCTY_TYPE:
					execv("../bin/divine.owcty",newArgv);
					maintenance(newArgv);
					_exit(EXEC_FAILED);	
				
				case DISTR_MAP_TYPE:
					execv("../bin/divine.distr_map",newArgv);
					maintenance(newArgv);				
					_exit(EXEC_FAILED);
								
				default:
					cerr << "\nError: Some unspecified error was occured!\n";
					maintenance(newArgv);				
					exit(NO_ALG_WAS_CHOSEN);
				}
	...
	cout << "==================================================\n";
	cout << "--------- NewBioDiVinE successfully done ---------\n";
	cout << "==================================================\n\n";
