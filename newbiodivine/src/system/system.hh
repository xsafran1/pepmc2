...
/*  
  //!Method for reading parametrisation file. It returns number of runnings for all possible combinations of parameters
  virtual int parametrisation(std::string fileName) { return 1; };
  
  //!Method that erases just one value for any multi-value parameter, 
  //!so that in next iteration will be used another combination of values of parameters 
  virtual void decrement_parametrisations_count() {};
  
  virtual void get_parameters(std::map<std::string, std::vector<double> >& p) {};
*/  
